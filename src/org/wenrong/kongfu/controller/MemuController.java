...
public class MemuController {
	...
	@ResponseBody
	@RequestMapping(value="/getMemus.action",produces="text/html;charset=UTF-8;")
	public String getMemus(@RequestBody String param){
		
		JSONObject object = JSON.parseObject(param);
	
		Integer categoryId = object.getInteger("categoryId");
		
		List<Memu> allMemuByCategoryId = memuService.getAllMemuByCategoryId(categoryId);
		
		return JSON.toJSONString(allMemuByCategoryId);
	}
